...
module.exports = {
  [`POST ${ApiPrefix}/user/login`](req, res) {
    console.log(req.body, req.body.profile.email)

    if (req.body.authResult && req.body.authResult.accessToken) {
      const now = new Date()
      now.setDate(now.getDate() + 1)
      res.cookie(
        'token',
        JSON.stringify({
          id: req.body.profile.email,
  ...
  [`GET ${ApiPrefix}/user`](req, res) {
    ...
    if (response.success) {
      var profile = JSON.parse(cookies.token)['profile']
